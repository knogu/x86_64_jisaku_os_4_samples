		  ...
		  void *_fs_start)
{
	...
	unsigned char cur_cap_addr = get_pci_conf_reg(0, nic_dev_num, 0, 0x34) & 0xffu;
	int i = 0;
	while (cur_cap_addr != 0 && i < 64) {
		puts("  CAP ADDR ");
		puth(cur_cap_addr, 2);
		puts(" ");
		union CapabilityHeader header;
		header.data = get_pci_conf_reg(0, nic_dev_num, 0, cur_cap_addr);
		if (header.bits.cap_id == 0x05) {
			msi_cap_addr = cur_cap_addr;
		} else if (header.bits.cap_id == 0x11) {
			// msix_cap_addr = cur_cap_addr;
		}
		cur_cap_addr = header.bits.next_ptr;
		i += 1;
	}
	...
	set_intr_desc_type(NIC_INTR_NO, nic_handler, GetCS());
	intr_load();
